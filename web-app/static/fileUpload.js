...
export function setupFileUpload() {
    ...
    saveIcon.onclick = function () {
        if (!fileInput.files.length || saveIcon.disabled) return;
        const file = fileInput.files[0];
        const formData = new FormData();
        formData.append("file", file);

        saveIcon.textContent = "⏳";
        saveIcon.disabled = true; // Deshabilita para evitar doble click

        fetch(`${BACKEND_URL}/api/inference/upload`, { // Corrige el endpoint
            method: "POST",
            body: formData
        })
            .then(async res => {
            ...
            .finally(() => {
                saveIcon.disabled = false; // Rehabilita el botón
            });
    };
}
