...
public class RagQueryRequest {
    ...
    @Schema(
        ...
        example = "¿Cuáles son los componentes principales de la arquitectura RAG?",
        required = true
    )
    private String query;

    @Schema(
        description = "ID de conversación para mantener contexto entre consultas",
        example = "conv-123456"
    )
    ...
    @Schema(
        ...
        example = "0.7", defaultValue = "0.7"
    )
    private double minSimilarity = 0.7;

    @Schema(
        description = "Incluir documentos recuperados en la respuesta",
        defaultValue = "true"
    )
    private boolean includeMatches = true;

    @Schema(
        description = "Prompt personalizado para mejorar la respuesta del modelo",
        example = "Responde de manera concisa y profesional"
    )
    private String customPrompt;

    @Schema(
        description = "ID del documento fuente a utilizar como filtro",
        example = "plan-carrera"
    )
    private String documentId;

    @Schema(
        description = "Metadatos adicionales para filtrar los documentos relevantes",
        example = "{\"organizacion\":\"Accenture\"}"
    )
